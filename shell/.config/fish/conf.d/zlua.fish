if status is-interactive
    # Fish-native `z.lua` initialization from the old `.zshrc` setup.
    if test -f "$HOME/z.lua"; and command -q lua
        set -l zlua_luaexe (command -s lua 2>/dev/null | string collect)
        if test -n "$zlua_luaexe"
            set -gx ZLUA_SCRIPT "$HOME/z.lua"
            set -gx ZLUA_LUAEXE "$zlua_luaexe"

            if not functions -q _zlua
                function _zlua
                    set -l arg_mode ""
                    set -l arg_type ""
                    set -l arg_subdir ""
            ...
            if not set -q _ZL_CMD
                set -gx _ZL_CMD z
            end

            if not functions -q "$_ZL_CMD"
                alias "$_ZL_CMD"=_zlua
            end

            if not functions -q _zlua_precmd
                function _zlua_precmd --on-event fish_prompt
                    _zlua --add "$PWD" 2> /dev/null &
                end
            end

            if not functions -q _z_complete
